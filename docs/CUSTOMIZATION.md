...
- [Configuring Hermes Tools](#configuring-hermes-tools)
- [Modifying the Agent Personality](#modifying-the-agent-personality)
- [LibreChat Customization](#librechat-customization)
...
graph TB
    subgraph "Configuration (no code changes)"
        ...
        T["Hermes Tools<br/>(config.yaml)"]
        LC["LibreChat Config<br/>(librechat.yaml)"]
    end
...
sequenceDiagram
    ...
    Worker->>Hermes: POST /v1/responses (system prompt + content)
    Note over Hermes: Agent may invoke web/file/vision tools
```
...
- `opencode-go` -- OpenCode Go (default; used for GLM-5)
- `openrouter` -- [OpenRouter](https://openrouter.ai) (aggregates many LLM providers)

### Vision provider (OpenRouter)

The Hermes vision tool uses a separate OpenRouter configuration for image analysis, regardless of the main provider setting. It uses **Gemini 3.1 Flash Lite** by default. To change the vision model, update `services/hermes/config.yaml`:

```yaml
vision:
  provider: openrouter
  model: google/gemini-flash-1.5-8b  # or any vision-capable model on OpenRouter
...
Hermes is equipped with three built-in tools: **web search** (Tavily), **file operations**, and **vision** (OpenRouter Gemini). These are configured in `services/hermes/config.yaml`.

### Tool overview

| Tool | Provider | Purpose | Requires |
|---|---|---|---|
| **web** | Tavily | Web search for factual verification, research | `TAVILY_API_KEY` |
| **file** | Local | Read files from the upload volume | None (mounted volume) |
| **vision** | OpenRouter (Gemini 3.1 Flash Lite) | Analyze images in submissions | OpenRouter API key |

### Enabling / disabling tools

Edit `services/hermes/config.yaml` to enable or disable specific tools:
...
tools:
  ...
  vision:
    enabled: true
    provider: openrouter
    model: google/gemini-flash-1.5-8b-lite
    api_key: ${OPENROUTER_VISION_API_KEY}
```

To disable a tool, set `enabled: false`. The agent will not attempt to use disabled tools.

### Tavily web search

The web search tool uses the [Tavily API](https://tavily.com) for grounded web search. To configure:

1. Get a Tavily API key at https://tavily.com
2. Add to `infra/shared/.env`:
   ```bash
   TAVILY_API_KEY=tvly-your-key-here
   ```
3. The Hermes container reads `TAVILY_API_KEY` from the environment automatically

The agent uses web search when it needs to verify factual claims, look up citations, or research context not present in the submission.

### File tool

The file tool allows the agent to read uploaded files directly. Files uploaded via `POST /api/submissions` are stored in the `upload_data` Docker volume, which is also mounted into the Hermes container.
...
tools:
  file:
    enabled: true
    allowed_paths:
      - /app/uploads  # Must match UPLOAD_DIR in api container
```

### Vision tool (OpenRouter Gemini)

The vision tool enables the agent to analyze images embedded in or attached to submissions. It uses OpenRouter's API to route requests to Gemini 3.1 Flash Lite:
...
Hermes supports custom tools via its plugin system. To add a custom tool:

1. Define the tool in `services/hermes/config.yaml` under `custom_tools`
2. Implement the tool handler (Hermes-specific extension API)
3. Restart the Hermes container

Consult the Hermes documentation for the custom tool API.
...
| **Tool Usage Guidelines** | When to use web search, file reading, or vision analysis |
...
- Use the web tool to look up unfamiliar libraries or frameworks when needed.
```
...
LibreChat v0.8.4 can be customized via its configuration file and environment variables.

### Connecting LibreChat to a different Hermes endpoint

By default, LibreChat connects to the `hermes-gateway` container on port 8642. To change this, update `LIBRECHAT_HERMES_URL` and `LIBRECHAT_HERMES_KEY` in `infra/shared/.env`:

```bash
LIBRECHAT_HERMES_URL=http://hermes-gateway:8642
LIBRECHAT_HERMES_KEY=your-hermes-api-key  # Must match HERMES_API_KEY
```

### LibreChat model configuration

LibreChat's model list is configured in `services/librechat/librechat.yaml`. To add or modify available models:
...
endpoints:
  custom:
    - name: "Hermes Agent"
      ...
      baseURL: "http://hermes-gateway:8642/v1"
      models:
        default: ["glm-5"]
        fetch: false
      titleConvo: true
...
Update SOUL.md to emphasize essay-specific evaluation. Enable the web tool to verify citations.
...
3. Enable the file tool so the agent can read source files directly
4. Optionally: add a new activity that runs the code in a sandbox and includes test results in the prompt
...
3. Enable the vision tool for image-heavy documents (PDFs with diagrams, charts)
4. The file upload functionality already supports this use case
...
3. Each stage can use a different prompt/rubric and different tool configuration
