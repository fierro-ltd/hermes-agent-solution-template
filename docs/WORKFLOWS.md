...
class AgentFeedback:          # AI evaluation result
    ...
    weaknesses: list[str]
    reasoning: str
    agent_trace: dict | None  # Full trace from /v1/responses (steps, usage, tools_used)
...
The core AI grading activity. Calls the Hermes gateway to evaluate a student submission and captures the full agent trace.
...
4. Calls `POST {HERMES_API_URL}/v1/responses` with the system prompt and submission content (falls back to `/v1/chat/completions` if `/v1/responses` is unavailable)
5. Parses the structured JSON response (`suggested_score`, `strengths`, `weaknesses`, `reasoning`)
6. Extracts agent trace metadata: `steps[]`, `usage{}`, `session_id`, `duration_seconds`, `tools_used[]`, `model`
7. Stores both the feedback and the trace in the database

**Returns:** `AgentFeedback` dataclass (including `agent_trace` dict)
...
The `evaluate_submission` activity captures the full agent execution trace when using the `/v1/responses` Hermes endpoint. This trace is stored in `reviews.agent_trace` (JSONB) and surfaced to the professor through the frontend's collapsible accordion.

### How trace capture works

```mermaid
flowchart TD
    A["evaluate_submission activity"] --> B["Call Hermes /v1/responses"]
    B --> C{Response includes trace?}
    C -->|"Yes (/v1/responses)"| D["Extract trace metadata"]
    C -->|"No (fallback /v1/chat/completions)"| E["trace = null"]
    D --> F["steps[] — reasoning + tool calls"]
    D --> G["usage{} — token counts"]
    D --> H["tools_used[] — web/file/vision"]
    D --> I["session_id, model, duration_seconds"]
    F --> J["Store in reviews.agent_trace JSONB"]
    G --> J
    H --> J
    I --> J
    E --> J
...
The trace is retrieved via `GET /api/submissions/{id}/trace` and rendered as a collapsible accordion in the submission detail view. Each step is displayed with its type icon, content, and timestamp. Token usage and tools used are shown in a summary header.

---

## Streaming Flow

The SSE streaming feature enables real-time visibility into the Hermes evaluation as it happens. FastAPI acts as a proxy between the browser and Hermes.
...
sequenceDiagram
    ...
    Note over API,Browser: text/event-stream connection opened
    API->>Hermes: POST /v1/chat/completions (stream=true)
    Hermes->>LLM: Streaming request
    LLM-->>Hermes: Token delta events
    Hermes-->>API: SSE token events
    API-->>Browser: Proxy SSE events

    Note over Browser: react-markdown renders<br/>accumulated content live
...
The streaming endpoint (`GET /api/submissions/{id}/stream`) is independent of the Temporal workflow. It opens a new Hermes SSE stream for display purposes -- the actual grading result is captured by the worker activity and stored in the database.
...
flowchart TD
    ...
    Loop --> Eval["Activity: evaluate_submission<br/>Calls Hermes /v1/responses → LLM<br/>Captures agent trace"]
...
The professor can take minutes, hours, or days to review the submission. They use the split-view UI in the React frontend to see the submission content alongside the AI's feedback (score, strengths, weaknesses, reasoning). They can also view the full agent trace in a collapsible accordion showing each reasoning step, tool call, and token usage.
...
|---|---|---|---|
| `evaluate_submission` | 180 seconds | 25 minutes | LLM calls can take 30-60 seconds; trace capture adds overhead |
...
- [Architecture](./ARCHITECTURE.md) -- How Temporal fits in the overall system, SSE streaming, agent trace
- [API Reference](./API.md) -- Endpoints that start and signal workflows, stream and trace endpoints
- [Data Model](./DATA_MODEL.md) -- Database tables updated by workflow activities, agent_trace schema
