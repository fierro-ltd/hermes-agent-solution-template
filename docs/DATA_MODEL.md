...
- [MongoDB (LibreChat)](#mongodb-librechat)
...
The template uses a single PostgreSQL 15 instance with **two logical databases**, plus a separate MongoDB 8.0 instance for LibreChat:
...
|---|---|---|---|
| `temporal` | PostgreSQL 15 | `temporal` | Temporal Server workflow state. Created automatically by `temporalio/auto-setup`. Do not modify. |
| `app_db` | PostgreSQL 15 | `app_user` | Application data: submissions, reviews, settings, and authentication tables. |
| `librechat` | MongoDB 8.0 | `librechat` | LibreChat conversation history, messages, and user data. Managed entirely by LibreChat. |
...
- Host: `mongodb` (Docker) or `localhost:27017` (host)
- Used exclusively by LibreChat; do not interact with it directly from application code
...
erDiagram
    ...
    reviews {
        ...
        jsonb agent_trace "nullable, DEFAULT NULL"
...
Stores AI evaluation results and professor decisions in the grading demo. A submission can have multiple reviews (one per evaluation cycle). The `agent_trace` column stores the full Hermes agent execution trace from the `/v1/responses` API.
...
| `agent_trace` | `JSONB` | Yes | `NULL` | Full agent execution trace (steps, tool calls, token usage, timing). Null if Hermes fell back to `/v1/chat/completions`. |
...
When `agent_trace` is `NULL`, the evaluation used the `/v1/chat/completions` fallback and no trace metadata was captured.
...
| Key | Description |
|---|---|
| `hermes_provider` | LLM provider name (e.g., `opencode-go`) |
| `hermes_model` | LLM model name (e.g., `glm-5`) |
...
MongoDB 8.0 runs as the `mongodb` container and is used exclusively by LibreChat for storing conversation data. The main application (FastAPI, workers) does not interact with MongoDB.

**Container:** `mongodb` (port `127.0.0.1:27017`)
**Database:** `librechat`
**Managed by:** LibreChat v0.8.4 via Mongoose ODM

### Collections (managed by LibreChat)

| Collection | Purpose |
|---|---|
| `users` | LibreChat user accounts (separate from better-auth users) |
| `conversations` | Chat conversation metadata and settings |
| `messages` | Individual chat messages with role, content, and timestamps |
| `files` | Uploaded file metadata |
| `sessions` | LibreChat session tokens |

**Important:** Do not write to MongoDB directly from application code. LibreChat manages its own schema and migrations. These collections are listed for informational purposes only.

### Data isolation

LibreChat data in MongoDB is completely separate from the main application data in PostgreSQL:

```mermaid
graph LR
    subgraph "PostgreSQL (app_db)"
        S["submissions"]
        R["reviews (+ agent_trace)"]
        A["app_settings"]
        U["user / session / account"]
    end

    subgraph "MongoDB (librechat)"
        LC_U["users"]
        LC_C["conversations"]
        LC_M["messages"]
    end
...
-- Reviews: query agent trace existence
CREATE INDEX reviews_agent_trace_idx ON reviews ((agent_trace IS NOT NULL));
```
...
| `submissions_created_at_idx` | Submission list (sortable table) | `ORDER BY created_at DESC` |
...
| `reviews_agent_trace_idx` | Trace existence check | `WHERE agent_trace IS NOT NULL` |
...
graph TD
    ...
    subgraph "Application (managed by API/Worker)"
        Submissions["submissions"]
        Reviews["reviews<br/>(agent_feedback + agent_trace)"]
...
### How `agent_trace` was added

The `agent_trace` column was added to the `reviews` table as a nullable JSONB column:

```sql
ALTER TABLE reviews ADD COLUMN agent_trace JSONB DEFAULT NULL;
```

For new installations, this column is included in `infra/shared/init-db.sql` from the start.
...
1. Keep a `migrations/` directory with numbered SQL files (e.g., `001_add_agent_trace.sql`)
...
### Get a submission with its latest review (including trace)

```sql
SELECT s.*, r.agent_feedback, r.agent_trace, r.suggested_score, r.final_score, r.decision
FROM submissions s
...
### Check which evaluations used Hermes tools

```sql
SELECT
  s.title,
  s.student_name,
  r.agent_trace -> 'tools_used' AS tools_used,
  r.agent_trace -> 'usage' -> 'total_tokens' AS total_tokens,
  r.agent_trace ->> 'duration_seconds' AS duration_seconds
FROM reviews r
JOIN submissions s ON s.id = r.submission_id
WHERE r.agent_trace IS NOT NULL
ORDER BY r.created_at DESC;
```

### Find evaluations that used web search

```sql
SELECT s.title, s.student_name, r.suggested_score
FROM reviews r
JOIN submissions s ON s.id = r.submission_id
WHERE r.agent_trace @> '{"tools_used": ["web_search"]}'::jsonb;
```
...
```sql
SELECT id, agent_feedback, agent_trace, suggested_score, final_score,
...
### Token usage statistics across all evaluations

```sql
SELECT
  avg((agent_trace -> 'usage' ->> 'total_tokens')::int) AS avg_tokens,
  max((agent_trace -> 'usage' ->> 'total_tokens')::int) AS max_tokens,
  avg((agent_trace ->> 'duration_seconds')::float) AS avg_duration_seconds
FROM reviews
...
- [API Reference](./API.md) -- Endpoints that read/write these tables, trace endpoint
- [Workflows](./WORKFLOWS.md) -- How workflows update submission and review records, trace capture
