  ...
  return (
    <div className="flex flex-col items-center gap-3 py-4">
      ...
      <p className="text-xs text-muted-foreground/60">
        See real-time progress in the panel →
      </p>
    </div>
  );
}
...
function SubmissionDetailPage() {
  ...
  const streamState = useSubmissionStream(id, sub?.status ?? "");
  const { data: traceData } = useSubmissionTrace(id, sub?.status ?? "");
        ...
        <div className="w-2/5">
          <ScrollArea className="h-full">
            <div className="p-6 space-y-6">
              ...
              {traceData && <AgentTrace trace={traceData} />}
