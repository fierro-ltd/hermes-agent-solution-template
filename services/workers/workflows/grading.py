    ...
    async def run(self, params: GradingParams) -> GradingResult:
        ...
        while True:
            ...
            try:
                ...
                # All retries exhausted — mark submission as failed gracefully
                workflow.logger.error(
                    "evaluate_submission permanently failed for %s: %s",
                    params.submission_id,
                    e,
                )
                self.status = "failed"
            ...
            review_id = await workflow.execute_activity(
                ...
                args=[
                    params.submission_id,
                    feedback_json,
                    feedback_dict.get("suggested_score", 0),
                    self.review_cycles,
                    trace_json,
                ],
